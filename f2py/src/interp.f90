...
!!******************************************************************
  ...
  real*8,parameter :: r1_5=1.5
  integer*4,parameter :: nlon=375
  integer*4,parameter :: nlat=255
  integer*4,parameter :: nsig=63
  integer*4 :: nxtilde,nytilde
  real*8    :: rlambda0,pihalf,sign_pole,atilde_x,atilde_y,btilde_x,btilde_y
  real*8    :: rlon_min_dd,rlon_max_dd,rlat_min_dd,rlat_max_dd
  real*8    :: rlon_min_ll,rlon_max_ll,rlat_min_ll,rlat_max_ll
  real*8    :: btilde_xinv,btilde_yinv
  
  integer*4 :: istart,jstart,lon1
   
  !arrays
  integer*4,allocatable,dimension(:,:) :: i0_tilde,j0_tilde
  integer*1,allocatable,dimension(:,:) :: ip_tilde,jp_tilde
  real*8,allocatable,dimension(:,:)    :: xtilde0,ytilde0
  real*8,allocatable,dimension(:,:)    :: cos_beta_ref,sin_beta_ref
  real*8,allocatable,dimension(:,:)    :: glon,glat,glon_an,glat_an


  type :: ob
     real*8 :: omf       ! (O-F)
     real*8 :: omf2      ! (O-F)^2
     real*8 :: ob        ! radar observation
     real*8 :: ges       ! Forecast
  end type ob
...
!!  integer*4                             ,intent(in   ) :: n !number of interpolatees (always 1)
!  real*8,dimension(nlat,nlon,nsig),intent(in   ) :: f
  real*8,dimension(  :,   :,    : ),intent(in   ) :: f
  real*8                           ,intent(in   ) :: dxin,dyin,dzin !dx=x location, dy=y location, dz=closest GRID UNIT height location
  real*8                          ,intent(  out) :: gout

! Declare local variables

  integer*4 m1,i,ix1,iy1,ix,ixp,iyp,n
  integer*4 iy,iz,izp,itime,itimep,j
  real*8 delx,delyp,delxp
  real*8 dely,delz,delzp
  
  real*8,dimension(1) :: dx,dy,dz,g  
  n=1
  dx(1)=dxin
  dy(1)=dyin
  dz(1)=dzin
  ...
  do i=1,n
     ...
     delzp=one-delz
!No time interpolation done so the second half of this has been removed from the original routine
...
! Declare passed variables
  integer*4                              ,intent(in   ) :: nlevs
  !real*8,dimension(nlat,nlon,nsig),intent(in   ) :: f
  real*8,dimension(  :  ,  : ,  : ),intent(in   ) :: f
  real*8,                                 intent(in   ) :: dxin,dyin
  !real*8,dimension(nlevs)         ,intent(  out) :: gout
  real*8,dimension(   :  )         ,intent(  out) :: gout

! Declare local variables
  integer*4 i,ix1,iy1,ix,ixp,iyp,n
  integer*4 iy,itime,itimep,j,k
  real*8 delx,delyp,delxp
  real*8 dely
  
  real*8,dimension(1) :: dx,dy
  real*8,dimension(nlevs,1) :: g  
  n=1
  dx(1)=dxin
  dy(1)=dyin
  g=zero  
  ...
  do i=1,n
     ...
     do k=1,nlevs
 !No time interpolation done so the second half of this has been removed from the original routine
        !g(k,i)=(f(ix,iy,k)*delxp*delyp+f(ixp,iy,k)*delx*delyp +  f(ix,iyp,k)*delxp*dely+f(ixp,iyp,k)*delx*dely)
        g(k,i)=(f(k,ix,iy)*delxp*delyp+f(k,ixp,iy)*delx*delyp +  f(k,ix,iyp)*delxp*dely+f(k,ixp,iyp)*delx*dely)
...
!
!! Declare passed variables
!  real*8,dimension(nlat,nlon),intent(in   ) :: fin
!  real*8,                                 intent(in   ) :: dxin,dyin
!  real*8,                                 intent(  out) :: gout
!
!! Declare local variables
!  integer*4 i,ix1,iy1,ix,ixp,iyp,n,nlevs
!  integer*4 iy,itime,itimep,j,k
!  real*8 delx,delyp,delxp
!  real*8 dely
!  
!  real*8,dimension(1) :: dx,dy
!  real*8,dimension(1,1) :: g
!  real*8,dimension(nlat,nlon,1) :: f
!  
!  f=zero
!  f(:,:,1)=fin    
!  n=1
...
!$$$ end documentation block
  ...
  !real*8   ,intent(in   ) :: glats(nlon,nlat),glons(nlon,nlat)
  real*8   ,intent(in   ) :: glats(  :  ,  : ),glons( :  ,  : )

  real*8,parameter:: rbig =1.0e30
  real*8 xbar_min,xbar_max,ybar_min,ybar_max
  real*8 clon,slon,r_of_lat,xbar,ybar
  integer*4 i,j,istart0,iend,iinc,itemp,ilast,jlast
  real*8,allocatable,dimension(:,:):: clata,slata,clona,slona
  !real*8:: clata(nlon,nlat),slata(nlon,nlat),clona(nlon,nlat),slona(nlon,nlat)
  real*8 clat0,slat0,clon0,slon0
  real*8 clat_m1,slat_m1,clon_m1,slon_m1
  real*8 clat_p1,slat_p1,clon_p1,slon_p1
  real*8 x,y,z,xt,yt,zt,xb,yb,zb
  real*8 rlonb_m1,clonb_m1,slonb_m1
  real*8 rlonb_p1,clonb_p1,slonb_p1
  real*8 crot,srot
...
!  define parameters for xy domain which optimally overlays input grid
  ...
  real*8   ,intent(in   ) :: rlons0(nx0,ny0),rlats0(nx0,ny0)

  integer*4,intent(  out) :: nx,ny
  real*8   ,intent(  out) :: xminout,xmaxout,yminout,ymaxout

  real*8,parameter:: r37=37.0

  real*8 area,areamax,areamin,extra,rlats0max,rlats0min,testlambda
  real*8 xthis,ythis
  integer*4 i,ip1,j,jp1,m

  real*8 coslon0(nx0,ny0),sinlon0(nx0,ny0)
  real*8 coslat0(nx0,ny0),sinlat0(nx0,ny0)
  real*8 count,delbar
  real*8 dx,dy,disti,distj,distmin,distmax
...
!$$$ end documentation block
  ...
  integer*4,intent(inout) :: ilast,jlast
  integer*4,intent(  out) :: i0,j0
  integer*1,intent(  out) :: ip,jp
  integer*4,intent(in   ) :: nx0,ny0
  real*8   ,intent(in   ) :: x,y
  real*8   ,intent(in   ) :: x0(nx0,ny0),y0(nx0,ny0)
 
  real*8 dista,distb,dist2,dist2min
  integer*4 i,inext,j,jnext
...
! !INPUT PARAMETERS:
    ...
    real*8,intent(in   ) :: u0,v0        ! earth wind component
    real*8,intent(in   ) :: rlon0        ! earth   lon (radians)
    real*8,intent(in   ) :: x,y          ! local x,y coordinate (grid units)

! !OUTPUT PARAMETERS:

    real*8,intent(  out) :: u,v          ! rotated coordinate of winds
...
!-------------------------------------------------------------------------
  ...
  real*8 beta,delx,delxp,dely,delyp

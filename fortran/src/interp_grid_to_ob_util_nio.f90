...
! Set up constants
       ...
       regional=.false.
       if(.not.regional) then ! line 1040
         ! This is global run
       ...
       if(regional) then
          ...
          do k=1,nlon
            do i=1,nlat
               ...
               if(glat(k,i) > 30.6 .and. glat(k,i) < 30.8)then
               if(glon(k,i) > 262 .and. glon(k,i) < 263)then
                   write(*,*) glat(k,i), glon(k,i), k,i
               end if
               end if
            end do
          end do
...
subroutine tintrp2a(fin,gout,dxin,dyin,nlevs)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    tintrp2a
!   prgmmr: parrish          org: np22                date: 1990-10-11
...
! Declare passed variables
  ...
  real(r_kind),dimension(nlon,nlat,nlevs,1),intent(in   ) :: fin
...
! Declare passed variables
  real(r_kind),dimension(3,3),intent(in   ) :: fin
  real(r_kind),                                 intent(in   ) :: dxin,dyin
  real(r_kind),                                 intent(  out) :: gout
...
! Declare local variables
  ...
  real(r_kind) :: dx,dy
  real(r_kind) :: g
  real(r_kind),dimension(3,3) :: f

  ! lippi created this from single_level for efficiency. Instead of
  ! interpolating from full grid, we only need a 3x3 grid. It's much faster. 
  f=zero
  f(:,:)=fin    
  dx=dxin
  dy=dyin
  ...
  delxp=one-delx; delyp=one-dely
     g=(f(ix,iy)  *delxp*delyp&
       +f(ixp,iy) *delx *delyp&
       +f(ix,iyp) *delxp*dely &
       +f(ixp,iyp)*delx *dely)
  
  gout=g  !carley add since we are only interested in 1 interpolatee (n)
  return
end subroutine tintrp2a_single_level_sliced
...
! Declare local variables
  ...
  real(r_kind) :: dx,dy
  real(r_kind) :: g
  real(r_kind),dimension(nlon,nlat) :: f
!  real(r_kind),dimension(nlat,nlon) :: f
  ...
  ixp=ix+1; iyp=iy+1
  if(ix1==nlat) then !if(ix1==nlon) then
     ixp=ix
  end if
...
subroutine tintrp2a_single_level_notworking(fin,gout,dxin,dyin)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    tintrp2a
!   prgmmr: parrish          org: np22                date: 1990-10-11
...
! Declare local variables
  ...
  do i=1,n
     ...
     delxp=one-delx; delyp=one-dely
     do k=1,nlevs!No time interpolation done so the second half of this has been removed from the original routine
        g(k,i)=(f(ix,iy,k)  *delxp*delyp&
               +f(ixp,iy,k) *delx *delyp&
               +f(ix,iyp,k) *delxp*dely &
               +f(ixp,iyp,k)*delx *dely)
     end do ! end loop over vertical levs
  end do ! end loop over number of locations
  ...
  return
end subroutine tintrp2a_single_level_notworking

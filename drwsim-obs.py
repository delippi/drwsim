...
######### USER DEFINED SETTINGS ################################################# 
    factor=int(1)  # factor for upscaling the native resolution of model res.   #
    ...
    maxobrange=100000                                                           #
    azimuths=3#360                    #360 azimuths                               #
    ithin=120                         #radar data is very hires 1=full res        #
    gatespc=250*ithin               #drws are typically in 250 m gates.         #
    gates=maxobrange/gatespc        #100km/250m = 400 gates.                    #
...
##### END OF USER DEFINED SETTINGS ##############################################
    ...
    for rid in xrange(len(df.ID)):
       if(strip(df.ID[rid])==staid):
          ...
          drwpol=np.zeros(shape=(len(tilts),azimuths,gates)); drwpol.fill(-999.)
          ...
          for thistilt in tilts: # Loop for simulating drws.
             ...
             for thisazimuth in (xrange(azimuths)): #thetas=360
                ...
                widgets=[widgetstring+': [',Percentage(),' ',Timer(),'] ', Bar(),' (', ETA(), ') ']
                bar = ProgressBar(widgets=widgets)
                for gate in bar(xrange(gates)):         #gates=400 or 100km
                   igate+=1
                   inside=False
                   ...
                   if(inside):
                      ...
                      #thislat=np.radians(rlatglob)
                      #thislon=np.radians(rlonglob)
                      ...
                      ugesin=tintrp3(ges_u,dlat,dlon,dpres)
                      vgesin=tintrp3(ges_v,dlat,dlon,dpres)
                      wgesin=tintrp3(ges_w,dlat,dlon,dpres)
                      ...
                      if(dpres < zero or dpres > nsig): exit("something went wrong at "+str(framinfo.lineno))
          ...
          for tilt in tilts: # Loop for writing simulated drws to bufr file.
             ...
             for phi in xrange(azimuths):
                obs = bufr.missing_value*np.ones((len(obstr.split()),gates),np.float)
                hdr[11]= np.float(phi)  #ANAZ - ANTENNA AZIMUTH ANGLE
                i=0                   # counter
                r=minobrange          # meters
                rend=maxobrange       # meters - 400 gates for 100km
                while(r < rend):
                    obs[0,i] = r/gatespc             #DISTANCE (FROM ANTENNA TO GATE CENTER) IN UNITS OF 250M 
                    obs[1,i] = drwpol[alpha,phi,r/gatespc]  #DOPPLER MEAN RADIAL VELOCITY 
...
#    print("Check grdcrd for corectness...")
...
#    print("Check isrchf for corectness...")
    if(flg==ione):
       for k in range(nx1):
           if(y<=x[k]): 
               isrchf=k
...
#    print("Check tintrp2a for correctness...")
    ...
    #ix=ix1+1#-istart+2
    #iy=iy1+1#-jstart+2
    #if(iy<1): #Double check
    #   iy1=iy1+nlon
    #   iy=iy1+1#-jstart+2
    #if(iy>lon1+1): #Double check
    #   iy1=iy1-nlon
    #   iy=iy1+1#-jstart+2 
    #if(ix1==nlat): #Double check
    #   ixp=ix
        ix=ix1
        iy=iy1
...
#  print("Make sure to check tll2xy for correctness...")
  diag=False
...
def tintrp31(f,dx,dy,dz,obstime):
    nlat,nlon,nsig=1440,1728,63 
    istart=0
    jstart=0
    delt=obstime-obstime
    #input argument list:
    #  f        - input interpolator
    #  dx,dy,dz - input x,y,z-coords of interpolation points (grid units)
    #  obstime  - time to interpolate to
    ...
    iy1=int(dy)
    iz=int(dz)
    ix1=max(0,min(ix1,nlat-1)); iz=max(0,min(iz,nsig-1))
    delx=dx-float(ix1)
    dely=dy-float(iy1)
    delz=dz-float(iz)
    ...
    print(iz,iy,ix)
    g =  (f[itime ,iz ,iy ,ix ]*delxp*delyp*delzp
        + f[itime ,iz ,iy ,ixp]*delx *delyp*delzp
        + f[itime ,iz ,iyp,ix ]*delxp*dely *delzp
        + f[itime ,iz ,iyp,ixp]*delx *dely *delzp
        + f[itime ,izp,iy ,ix ]*delxp*delyp*delz
        + f[itime ,izp,iy ,ixp]*delx *delyp*delz
        + f[itime ,izp,iyp,ix ]*delxp*dely *delz
        + f[itime ,izp,iyp,ixp]*delx *dely *delz)
    return(g)
...
def get_lat_lon(azm,thistiltr,gate,radar_lat,radar_lon):
    x=gate*250.*np.cos(thistiltr)
    return(dlat,dlon)

def get_ijk(lat,lon,height):
    return 0

def model_to_ob(rw_operator):
    return 0

def bufrize(azm,radarid,gate,elev,tilt):
    return 0
